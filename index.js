...
const PRODUCT_LIST_KEYBOARD = `List of products ${String.fromCodePoint(
  0x1f4f0
)}`;
const PRODUCT_DELETE_KEYBOARD = `Delete certain product. ${String.fromCodePoint(
  0x274c
)}`;
const PRODUCT_EDIT_KEYBOARD = `Edit product ${String.fromCodePoint(0x1f4f0)}`;
const PRODUCT_CREATE_KEYBOARD = `Create product. ${String.fromCodePoint(
  0x2705
)}`;
const PRODUCT_GET_KEYBOARD = `Get some product. ${String.fromCodePoint(
  0x1f504
)}`;
const PRODUCT_STOP_EDITING_KEYBOARD = `Stop editing ${String.fromCodePoint(
  0x274c
)}`;
...
const editing_keyboard = [[PRODUCT_STOP_EDITING_KEYBOARD], [HOME_KEYBOARD]];
...
const USER_PATH = "http://localhost:8080/";
let userToRegister;
            ...
            bot.sendPhoto(id, item.imgUrl, {
              ...
              reply_markup: {
                inline_keyboard: [
                  ...
                  [
                    {
                      text: `Delete this product ${String.fromCodePoint(
                        0x274c
                      )}`,
...
bot.onText(new RegExp(`${PRODUCT_EDIT_KEYBOARD}(.*)`), (message) => {
  ...
  if (entityId !== undefined) {
    bot.addListener("message", (msg) => {
      bot.sendMessage(
        id,
        "Enter field which you want to edit in the next format - fieldName:fieldValue",
        {
          reply_markup: {
      ...
      switch (fieldName) {
        case PRODUCT_NAME_FIELD:
          ...
          bot.sendMessage(
            id,
            "Do you want to continue editing ? Type 'End' if you want to save edited values or if you don't, just keep typing properties and their values."
          );
          break;
        ...
        case "End":
          ...
          bot.removeListener("message");
          axios
            .post(`${PATH}/update/${entityId}`, updatedEntity)
            .then((res) => {
